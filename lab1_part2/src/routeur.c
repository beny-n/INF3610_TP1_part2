...
//CPU_TS64 ts_delta_mutex_acc = 0L;
//CPU_TS64 ts_delta_sem_acc = 0L;
...
//	int packGenQty = (rand() % 255);
	...
	while(true) {
		...
		else
		{
			...
			isGenPhase = true;
			do { packGenQty = 255; } while (packGenQty == 0); //(rand() %255);
...
#elif DELAI_1_2 == 1
				do { WAITFORTICKS = (rand() %3); } while (WAITFORTICKS == 0);
				actualticks = OSTimeGet(&err);
				while(WAITFORTICKS + actualticks > OSTimeGet(&err));
#endif
