...
#if (UCOS_TS_DRIVER == UCOS_TS_AXITIMER)
#include  <ucos_axitimer.h>
#endif /* #if (UCOS_TS_DRIVER == UCOS_TS_AXITIMER) */
...
void  CPU_TS_TmrInit (void)
{
    ...
    TmrHandle = AXITimer_Init(UCOS_TS_DEVICE_ID);

    AXITimer_OptSet(TmrHandle, 0, AXITIMER_BIT_TCSR_CASC);

    tmr_freq = AXITimer_FreqGet(TmrHandle);
    CPU_TS_TmrFreqSet(tmr_freq);
...
CPU_TS_TMR  CPU_TS_TmrRd (void)
{
    ...
    ts_tmr_high_b = *((CPU_REG32 *)(UCOS_TS_BASE_ADDR + 0x18u));

    do {
        ts_tmr_high_a = ts_tmr_high_b;
        ts_tmr_low    = *((CPU_REG32 *)(UCOS_TS_BASE_ADDR + 0x08u));
        ts_tmr_high_b = *((CPU_REG32 *)(UCOS_TS_BASE_ADDR + 0x18u));
...
#if (UCOS_TS_DRIVER == UCOS_TS_AXITIMER)
    res = (CPU_INT32U)(ts_cnts / (CPU_TS_TmrFreq_Hz / 2));
#else
    res = 0;
#endif
...
#if (UCOS_TS_DRIVER == UCOS_TS_AXITIMER)
    res = ts_cnts / (CPU_TS_TmrFreq_Hz / 2);
#else
    res = 0;
#endif
